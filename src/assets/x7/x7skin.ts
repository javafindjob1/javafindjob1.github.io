...
export const x7skin: Skin = {
  ...
  locArr: [
    [0, 1903],
    [0, 263, 416, 545, 717, 893, 1026, 1207, 1400, 1517, 1683, 1903],
    [0, 179, 351, 529, 717, 893, 1050, 1207, 1405, 1534, 1707, 1903],
    [0, 77, 323, 488, 717, 893, 1057, 1257, 1467, 1584, 1780, 1903],
    [0, 77, 250, 488, 717, 893, 1057, 1257, 1467, 1820, 1903],
    [0, 1903],
  ],
  ...
  heroImg: ['https://p3-sign.toutiaoimg.com/tos-cn-i-eyfl5e3fp8/41a450b1c54342ae86238c8999beb0dd~tplv-obj.image?lk3s=993df49e&traceid=20250702190127792DC062C3CF261C2BC7&x-expires=2147483647&x-signature=DdC%2FIHwLLuJbvov0B%2BfYqmh9ucY%3D',
    '/x7/x7-heros.webp',
  ],
  heroPanelImg: ['https://image-tt-private.toutiao.com/tos-cn-i-eyfl5e3fp8/6e4471584e924403b106d9143cf88dfb~tplv-obj.image?lk3s=993df49e&policy=eyJ2bSI6MywidWlkIjoiMjM2NTc0MDc4Mjk4ODU4OSJ9&traceid=2025121419291301154FAD1036116E48BC&x-orig-authkey=5a21e4afda5945d9a206a695e4c78a63&x-orig-expires=2147483647&x-orig-sign=kHxlwBvl9rcZMkRG9v0LJuGvxKQ%3D',
    '/x7/x7-imgs/out.webp',
  ],
  ...
  calcAbility(unitId, p1, p2, i2, j2, rate) {
    ...
    rate = rate || 0.84
    return `background: url('${this.heroPanelImg[0]}') no-repeat -${offsetx * rate}px -${offsety * rate}px / ${this.heroPanelImgWidth * rate}px ${this.heroPanelImgHeight * rate}px`
  ...
  calcHead(unitId, p1, p2) {
    ...
    return `background: url('${this.heroPanelImg[0]}') no-repeat ${offsetx}px ${offsety}px / ${this.heroPanelImgWidth * 0.7}px ${this.heroPanelImgHeight * 0.7}px`
  },
  calcDef(defType) {
    const name = "InfoPanelIconArmor" + defType.substring(0, 1).toUpperCase() + defType.substring(1)
    const i = this[name as keyof Skin] as number
  ...
  calcStyle(i) {
    const rate = 0.5
    return `background: url('${this.heroPanelImg[0]}') no-repeat -${i * 70 * rate}px -${(this.heroPanelImgHeight - 64) * rate}px / ${this.heroPanelImgWidth * rate}px ${this.heroPanelImgHeight * rate}px`
  },
